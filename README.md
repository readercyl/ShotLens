...
- 把选区内识别到的非中文自然语言翻译为简体中文；中文、数字、代码和链接保持不变，按结果窗内的完整语义流排版
...
- OCR 坐标只用于阅读顺序、栏位判断、样式和原文字背景恢复；译文在结果窗内整体排版，正常使用四周背景像素重建原文字区域，重建不可用时只在原始范围内使用采样背景兜底
...
翻译输入使用最简单的稳定编号文本行，不再要求模型生成 JSON。一次请求会携带同一选区内的语义块，并可把框选外围最多 8 条、600 字符的 OCR 文本作为只读语境用于短词消歧；返回优先按编号文本行解析，同时兼容编号对象、数组、代码块、常见箭头分隔、解释前缀、SSE 边缘残片、控制标记和不完整 JSON。超长语义块会在自然句界处分段后再合并回原坐标，超大选区按受控批次分批并最多并发两路；首批可靠结果会先覆盖显示，遇到限流后以单路恢复缺项。只有少量高置信、低歧义 UI 词会在本地完成，多义词仍交给模型结合语境判断；同一 App 会话内完全相同的完整成功翻译最多缓存 128 批，退出后自动清空且不写入磁盘。程序会确定性检查已有中文、数字、URL 和反引号代码没有丢失，并拒绝各语种原文照抄；产品名、型号和标识符允许合理保留原文。DeepSeek 官方接口会关闭思考模式并限制回复长度，避免翻译前生成无用推理。

浮框里的“重新翻译”会复用已经完成的 OCR、语义分组和框选外围语境，只重新调用翻译 API 并重新排版，不会再次截图或重复 OCR。

ShotLens 默认继续使用 macOS Vision 在本机 OCR，并动态使用当前系统支持的识别语言；原图准确识别作为主结果，单词或单行高置信结果直接返回，分散小文本和低置信大图才追加轻量灰度、对比度与锐化增强识别。截图像素仍不上传。框选较小时会自动增加 OCR 上下文，但最终只处理原始框选范围内的内容。每个包含非中文自然语言的完整语义块会发送给 API，模型翻译其中所有支持的外语并保留中文、数字、代码和链接后整体排版。
...
- `check-compact-ui.sh`：验证 API 详情默认折叠、更新检测文字按钮和结果窗译文排版约束。
