...
- 固定把英文翻译为简体中文；中文不发送给 API，也不参与原位覆盖
...
ShotLens 默认继续使用 macOS Vision 在本机 OCR：原图准确识别作为主结果，轻量灰度、对比度和锐化增强图只补回浅色漏字，截图像素仍不上传。框选较小时会自动增加 OCR 上下文，但最终只接纳原始框选范围内的英文片段；中文不会发送给 API，也不会被覆盖。云端 OCR 只有在提供稳定逐块坐标契约，并且隐私、网络成本和端到端延迟实测优于本地 Vision 时才重新评估。
