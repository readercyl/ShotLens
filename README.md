...
- 固定把英文翻译为简体中文；孤立英文尽量原位替换，轻量中英数字混排按完整语义重排，已有中文和数字保持不变
...
7. 结果浮框外侧的透明 pin 图标会根据截图背景自动使用黑色或白色；未钉住时图标竖直，钉住后转为 45° 斜向。钉住并翻译成功后，下侧控制区会自动隐藏。
...
浮框里的“重新翻译”会直接复用当前 OCR 和语义分组结果，只重新调用翻译 API；OCR 尚未成功时才重新识别当前框选截图。整个过程不会再次截取屏幕，也不会捕获已经显示的译文或要求用户重新框选。

ShotLens 默认继续使用 macOS Vision 在本机 OCR：原图准确识别作为主结果，轻量灰度、对比度和锐化增强图只补回浅色漏字，截图像素仍不上传。框选较小时会自动增加 OCR 上下文，但最终只处理原始框选范围内的内容。中文占主体时只发送英文子范围；英文占主体且只夹少量中文或数字时，会把完整语义块发送给 API，并要求中文和数字原样保留后整体重排。
...
- `check-translation-content.sh`：验证完整中英文上下文、跨行语义分组和多栏隔离。
- `check-ocr-selection-filter.sh`：验证原图与增强图双路识别、浅色文字、完整中英文行，以及边界残字和纯符号排除。
...
ShotLens 会在本机完成截图、框选、OCR 和译文覆盖渲染。截图像素仅用于本地 OCR 和覆盖渲染；待翻译英文及必要的受保护中文、数字语义上下文会发送给你配置的 API 服务商。
